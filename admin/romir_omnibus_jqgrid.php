...
$indextext="";
if ($result = mysqli_query($conn, "SELECT * FROM omnibus WHERE language_id=".$lang)) {
								...
								<fieldset >
									<section>
										...
										<div class="inline-group">
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_omnibus_jqgrid.php?lang=1'" name="radio-inline" <?php if ($lang==1){ ?> checked="checked" <?php } ?>>
												<i></i>RUS</label>
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_omnibus_jqgrid.php?lang=2'" name="radio-inline" <?php if ($lang==2){ ?> checked="checked" <?php } ?>>
												<i></i>ENG</label>
...
function fechaReg(el) {
 ...
 //   var zipEditParams = $.extend(true, {}, editParams, {url: 'ajax.php'});
		...
		jQuery("#jqgrid").jqGrid({
			...
			datatype : "local",
			height : 'auto',
			colNames : ['Операции', 'ID',  'Название', 'Язык', 'Тип', 'Время начала', 'Подача вопросов до',  'Результат исследований', 'Видимость'],
			colModel : [
				...
				{ name : 'type', index : 'type', sortable : false, editable : true, edittype: "select", editoptions: { value: "1:Всероссийский опрос; 2:Онлайн опрос"}},
				{ name : 'start_day', index : 'start_day', editable : true, editoptions: {dataInit: fechaReg, readonly: 'readonly'}, editrules: {required: true, edithidden: true}, search: false }, 
				{ name : 'submission', submission : 'time', align : "right", editable : true }, 
				{ name : 'result', index : 'result', align : "right", editable : true }, 
