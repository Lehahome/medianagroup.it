...
include("inc/nav.php");
	...
	$newstext="";
	if ($result = mysqli_query($conn, "SELECT * FROM subscribe ORDER BY family, name")) {
		...
		foreach ($result as $row)	
		{
			...
			$newstext.="{
					...
					wtel : \"".addslashes($row["wtel"])."\",
					mtel : \"".addslashes($row["mtel"])."\",
...
// }; 
 //   var zipEditParams = $.extend(true, {}, editParams, {url: 'ajax.php'});
		...
		jQuery("#jqgrid").jqGrid({
			...
			datatype : "local",
			height : 'auto',
			colNames : ['ID', 'Фамилия', 'Имя Отчество', 'Название СМИ/компании', 'Рабочий телефон', 'Мобильный телефон', 'E-mail'],
			colModel : [
				...
				{ name : 'wtel', index : 'wtel', sortable : true, editable : true },
				{ name : 'mtel', index : 'mtel', sortable : true, editable : true },
