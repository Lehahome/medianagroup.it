...
$indextext="";
if ($result = mysqli_query($conn, "SELECT * FROM indexes_media WHERE language_id=".$lang." ORDER BY sort ASC")) {
...
foreach ($result as $row)	
{
	...
	$indextext.="{
			...
			period_geograpfy : \"".addslashes($row["period_geograpfy"])."\",
			ca : \"".addslashes($row["ca"])."\",
			cell1 : \"".addslashes($row["cell1"])."\",
			cell2 : \"".addslashes($row["cell2"])."\",
			cell3 : \"".addslashes($row["cell3"])."\",
			cell4 : \"".addslashes($row["cell4"])."\",
			cell5 : \"".addslashes($row["cell5"])."\",
			cell6 : \"".addslashes($row["cell6"])."\"
		},";
}
$result->close();
} 
								...
								<fieldset >
									<section>
										...
										<div class="inline-group">
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_jqgrid_media.php?lang=1'" name="radio-inline" <?php if ($lang==1){ ?> checked="checked" <?php } ?>>
												<i></i>RUS</label>
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_jqgrid_media.php?lang=2'" name="radio-inline" <?php if ($lang==2){ ?> checked="checked" <?php } ?>>
												<i></i>ENG</label>
...
<script type="text/javascript">
 ...
 //   var zipEditParams = $.extend(true, {}, editParams, {url: 'ajax.php'});
		...
		jQuery("#jqgrid").jqGrid({
			...
			datatype : "local",
			height : 'auto',
			colNames : ['Операции', 'ID', 'Сортировка', 'Язык',  'Название', 'Период, География', 'ЦА', 'Поле 1', 'Поле 2', 'Поле 3', 'Поле 4', 'Поле 5', 'Поле 6', 'Видимость'],
			colModel : [
				...
				{ name : 'period_geograpfy', index : 'period_geograpfy', editable : true }, 
				{ name : 'ca', index : 'ca', editable : true },
				{ name : 'cell1', index : 'cell1', align : "right", editable : true }, 
				{ name : 'cell2', index : 'cell2', align : "right", editable : true }, 
				{ name : 'cell3', index : 'cell3', align : "right", editable : true }, 
				{ name : 'cell4', index : 'cell4', align : "right", editable : true }, 
				{ name : 'cell5', index : 'cell5', align : "right", editable : true }, 
				{ name : 'cell6', index : 'cell6', align : "right", editable : true }, 
