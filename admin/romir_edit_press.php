...
include("inc/nav.php");
 ...
 $title="";
	...
	if ($result = mysqli_query($conn, "SELECT press.*, short_url.short_url 
	FROM press 
	LEFT JOIN short_url ON short_url.press_id=press.id
	WHERE press.id=".$editid)) {
	...
	<div id="content">
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				<h1 class="page-title txt-color-blueDark">
					...
					<i class="fa-fw fa fa-home"></i> 
						Главная 
					<span>>  
						<a href="<?=APP_URL?>/romir_press.php">Пресса о нас</a>
					<span>>  
						<?=$title;?>	
					</span>
				</h1>
		...
		<!-- widget grid -->
			...
			<div class="row">
				...
				if ($result = mysqli_query($conn, "SELECT * FROM press WHERE language_id=2 AND parent_press_id=".$_REQUEST["id"]." LIMIT 1")) {
					//echo "SELECT * FROM press WHERE language_id=2 AND parent_page_id=".$_REQUEST["id"]." LIMIT 1";
					//$result->fetch_array(MYSQLI_ASSOC);
					if (mysqli_num_rows($result)==0 && $language_id!=2) {
							
								?>
								
								<a href="<?=APP_URL?>/romir_add_press.php?lang=2&parent_press_id=<?php echo $_REQUEST["id"]; ?>" style="margin-bottom:20px; display:block; width:450px;" class="btn btn-info btn-lg">Добавить пресс-релиз на английском языке</a>	
				...
				?>		
                 ...
                 <form id="edit-form" class="smart-form" enctype="multipart/form-data"  method="post" action="<?=APP_URL?>/romir_update_press.php">
				 ...
				 <fieldset>
						 ...
						 <section>
							...
							<option <?php if ($press_active==1) {echo "selected";}?> value="1">Вкл</option>
							<option <?php if ($press_active==2) {echo "selected";}?> value="2">Выкл</option>
				 ...
				 <fieldset>
				  ...
				  <fieldset>
						 ...
						 <section>
							 <a onclick="UpdatePress();" class="btn btn-info btn-lg">Сохранить</a>
							 <a onclick="DeletePress();" class="btn btn-danger btn-lg pull-right">Удалить</a>				 
					  </section>
				 ...
				 <div id="delete_dialog">
					<p>
					Вы уверены в том, чтобы удалить этот пресс релиз?
					</p>
				</div>		
...
<script type="text/javascript">
	...
	function RealPressDelete()
	{
		var press_id=$("#editid").val();
		var data={"press_id":press_id, "oper":"delete_press"};
		$.ajax({
			  type: "POST",
			  ...
			  success: function(msg){
				 if (msg.result===true) {
					 location.href="<?=APP_URL?>/?press_deleted";
				 } 
				//console.log( msg );
			  }
			});
