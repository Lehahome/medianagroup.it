...
// 
 ...
 if (isset($short_url) && !empty($short_url) ) {
	...
	WHERE short_url.new_id=".$editid)) {
	  ...
	  if ($gotrec==1) {
		 ...
		 $query="UPDATE short_url SET  short_url='".$short_url."' WHERE  new_id=".$editid;
	  ...
	  } else
      {
		  ...
		  $query_insert="INSERT INTO short_url (new_id, short_url) VALUES (".$editid.", '".$short_url."')";
 ...
 if (isset($title) && !empty($title) ) {
		 ...
		 $query="UPDATE news SET  title='".$title."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
  ...
  if (isset($active) && !empty($active) ) {
		 ...
		 $query="UPDATE news SET  active=".$active." WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($keywords)) {
		 ...
		 $query="UPDATE news SET  keywords='".$keywords."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($description)) {
		 ...
		 $query="UPDATE news SET  description='".$description."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($publish_date) && !empty($publish_date) ) {
		 ...
		 $query="UPDATE news SET  publish_date='".$publish_date."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 // if (isset($url) && !empty($url) ) {
		 // $url="/news/".$url;
		 // $query="UPDATE news SET url='".$url."' WHERE  id=".$editid;
		 // if ($result = mysqli_query($conn, $query)) {
	 ...
	 if (isset($ckeditor) && !empty($ckeditor) ) {
		 ...
		 $query="UPDATE news SET  html='".$ckeditor."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
   ...
   // $file_new_name = '/home/romir18530/f.romir18530.nichost.ru/docs/upload/pics/';
    ...
    if( move_uploaded_file($file_name_tmp, $full_path) ) {
		$query="UPDATE news SET  small_pic='".$url."' WHERE  id=".$editid;
		//echo $query; exit;
		if ($result = mysqli_query($conn, $query)) {
		//$result->fetch_array(MYSQLI_ASSOC);
   ...
   if (isset($_FILES) && isset($_FILES['big_pic']) && !empty($_FILES['big_pic']['name']) ) {
    ...
    $error = '';
    if( move_uploaded_file($file_name_tmp, $full_path) ) {
		$query="UPDATE news SET  big_pic='".$url."' WHERE  id=".$editid;
		//echo $query; exit;
		if ($result = mysqli_query($conn, $query)) {
		//$result->fetch_array(MYSQLI_ASSOC);
