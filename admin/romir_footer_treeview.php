								...
								<fieldset >
									<section>
										...
										<div class="inline-group">
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_footer_treeview.php?lang=1'" name="radio-inline" <?php if ($lang==1){ ?> checked="checked" <?php } ?>>
												<i></i>RUS</label>
											<label class="radio">
												<input type="radio" onclick="location.href='<?=APP_URL?>/romir_footer_treeview.php?lang=2'" name="radio-inline" <?php if ($lang==2){ ?> checked="checked" <?php } ?>>
												<i></i>ENG</label>
								...
								<div class="tree smart-form">
									<ul>
										<li>
											...
											<?php if ($result = mysqli_query($conn, "SELECT * FROM header_menu WHERE active=1 AND parent=0  AND language_id=".$lang." ORDER BY sort")) {
											...
											{ ?><li>
													...
													<input class="parent_active" id="title_<?=$row["id"]?>" placeholder="Название" style="margin-left:15px;" value="<?=$row["title"]?>" type="text"/><input id="sort_<?=$row["id"]?>" placeholder="сортировка" style="margin-left:15px; width:90px; text-align:center; display:none;" value="<?=$row["sort"]?>" type="number"/>
													
													
													
												 
													 <ul>
														<?php if ($result_submenu = mysqli_query($conn, "SELECT * FROM header_menu WHERE  parent=".$row["id"]."  AND language_id=".$lang." ORDER BY sort")) {
									...
									<a onclick="UpdateDownMenu();" class="btn btn-info btn-lg">Сохранить</a>
								</div>
