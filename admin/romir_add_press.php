...
if (!empty($addpress)) {
 ...
 $parent_press_id=0;
 if (isset($_REQUEST["parent_press_id"]) && $_REQUEST["parent_press_id"]!="") {
	$parent_press_id=(int)$_REQUEST["parent_press_id"]; 
 }
 ...
 $new_url=$press_link."/".$new_url;
 
  $query="INSERT INTO press (keywords, description, title,publish_date,url,small_pic,big_pic,html,parent_press_id,language_id) VALUE ('".$keywords."', '".$description."','".$title."','".$publish_date."','".$new_url."','".$small_pic."','".$big_pic."','".$ckeditor."',".$parent_press_id.",".$language_id.")";
		 if ($result = mysqli_query($conn, $query)) {
			 
			 if (isset($short_url) && !empty($short_url)) {
					$last_id=mysqli_insert_id($conn);
					
					  $query_short_url="INSERT INTO short_url (short_url,press_id) VALUE ('".$short_url."', ".$last_id.")";
 ...
 //echo $query;
 //exit;
 header("Location: ".APP_URL."/romir_press.php"); 
...
$page_title = "Добавление пресс релиза";
		...
		<!-- widget grid -->
			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                 ...
                 <form id="add-form" class="smart-form" enctype="multipart/form-data"  method="post" action="<?=APP_URL?>/romir_add_press.php">
					...
					<fieldset>
						 ...
						 <section>
						   ...
						   <?php 
							$short_name="";
							$html="";
							if (isset($_GET["parent_press_id"])) {
							
								if ($result = mysqli_query($conn, "SELECT * FROM press WHERE id=".$_GET["parent_press_id"])) {
				 ...
				 <input type="hidden" value="<?php if (isset($_GET["parent_press_id"])) {echo $_GET["parent_press_id"];} ?>" id="parent_press_id" name="parent_press_id"/>
				 ...
				 <fieldset>
						 ...
						 <section>
								    <label class="label">Текст пресс релиза</label>
				  ...
				  <fieldset>
						 ...
						 <section>
				 <a onclick="AddNewPress();" class="btn btn-info btn-lg">Сохранить</a>	 
					  </section>
