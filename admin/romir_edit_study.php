	...
	<div id="content">
		...
		<!-- widget grid -->
			...
			<div class="row">
				...
				if ($result = mysqli_query($conn, "SELECT * FROM studies WHERE language_id=2 AND parent_study_id=".$_REQUEST["id"]." LIMIT 1")) {
					if (mysqli_num_rows($result)==0 && $language_id!=2) {
							
								?>
								
								<a href="<?=APP_URL?>/romir_add_study.php?lang=2&parent_study_id=<?php echo $_REQUEST["id"]; ?>" style="margin-bottom:20px; display:block; width:450px;" class="btn btn-info btn-lg">Добавить исследование на английском языке</a>	
				...
				?>									
                 ...
                 <form id="edit-form" class="smart-form" enctype="multipart/form-data"  method="post" action="<?=APP_URL?>/romir_update_study.php">
				 ...
				 <fieldset>
						 ...
						 <section>
							...
							<option <?php if ($study_active==1) {echo "selected";}?> value="1">Вкл</option>
							<option <?php if ($study_active==2) {echo "selected";}?> value="2">Выкл</option>
				 ...
				 <fieldset>
				  ...
				  <fieldset>
						 ...
						 <section>
							<a onclick="UpdateStudy();" class="btn btn-info btn-lg">Сохранить</a>
							<a onclick="DeleteStudy();" class="btn btn-danger btn-lg pull-right">Удалить</a>							
					  </section>
				...
				<div id="delete_dialog">
					<p>
					Вы уверены в том, чтобы удалить это исследовние?
					</p>
				</div>	
...
<script type="text/javascript">
	...
	function RealStudyDelete()
	{
		var study_id=$("#editid").val();
		var data={"study_id":study_id, "oper":"delete_study"};
		$.ajax({
			  type: "POST",
			  ...
			  success: function(msg){
				 if (msg.result===true) {
					 location.href="<?=APP_URL?>/?study_deleted";
				 } 
				//console.log( msg );
			  }
			});
