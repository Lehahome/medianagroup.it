...
// 
 ...
 $publish_date=$_REQUEST["publish_date"];
  ...
  if (isset($short_url) && !empty($short_url) ) {
	...
	FROM short_url 
	WHERE short_url.study_id=".$editid)) {
	  ...
	  if ($gotrec==1) {
		 ...
		 $query="UPDATE short_url SET  short_url='".$short_url."' WHERE  study_id=".$editid;
	  ...
	  } else
      {
		  ...
		  $query_insert="INSERT INTO short_url (study_id, short_url) VALUES (".$editid.", '".$short_url."')";
 ...
 if (isset($title) && !empty($title) ) {
		 ...
		 $query="UPDATE studies SET  title='".$title."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($active) && !empty($active) ) {
		 ...
		 $query="UPDATE studies SET  active=".$active." WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
...
if (isset($keywords)) {
		 ...
		 $query="UPDATE studies SET  keywords='".$keywords."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($description)) {
		 ...
		 $query="UPDATE studies SET  description='".$description."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 if (isset($publish_date) && !empty($publish_date) ) {
		 ...
		 $query="UPDATE studies SET  publish_date='".$publish_date."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
 ...
 // if (isset($url) && !empty($url) ) {
		 // $url="/studies/".$url;
		 // $query="UPDATE studies SET  url='".$url."' WHERE  id=".$editid;
		 // if ($result = mysqli_query($conn, $query)) {
	 ...
	 if (isset($ckeditor) && !empty($ckeditor) ) {
		 ...
		 $query="UPDATE studies SET  html='".$ckeditor."' WHERE  id=".$editid;
		 if ($result = mysqli_query($conn, $query)) {
   ...
   if (isset($_FILES) && isset($_FILES['small_pic']) && !empty($_FILES['small_pic']['name']) ) {
    ...
    if( move_uploaded_file($file_name_tmp, $full_path) ) {
		$query="UPDATE studies SET  small_pic='".$url."' WHERE  id=".$editid;
		//echo $query; exit;
		if ($result = mysqli_query($conn, $query)) {
		//$result->fetch_array(MYSQLI_ASSOC);
   ...
   if (isset($_FILES) && isset($_FILES['big_pic']) && !empty($_FILES['big_pic']['name']) ) {
    ...
    $error = '';
    if( move_uploaded_file($file_name_tmp, $full_path) ) {
		$query="UPDATE studies SET  big_pic='".$url."' WHERE  id=".$editid;
		//echo $query; exit;
		if ($result = mysqli_query($conn, $query)) {
		//$result->fetch_array(MYSQLI_ASSOC);
