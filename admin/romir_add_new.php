...
if (!empty($addnew)) {
 ...
 $parent_new_id=0;
 if (isset($_REQUEST["parent_new_id"]) && $_REQUEST["parent_new_id"]!="") {
	$parent_new_id=(int)$_REQUEST["parent_new_id"]; 
 }
 ...
 //$new_url="/news/".$new_url;
  ...
  $new_url=$new_link."/".$new_url;
 
  //$query="INSERT INTO studies (title,publish_date,url,small_pic,big_pic,html,parent_study_id,language_id
 
 
  $query="INSERT INTO news (keywords, description, title,publish_date,url,small_pic,big_pic,html,parent_new_id,language_id) VALUE ('".$keywords."', '".$description."', '".$title."','".$publish_date."','".$new_url."','".$small_pic."','".$big_pic."','".$ckeditor."',".$parent_new_id.",".$language_id.")";
		 if ($result = mysqli_query($conn, $query)) {
			 
			 if (isset($short_url) && !empty($short_url)) {
					$last_id=mysqli_insert_id($conn);
					
					  $query_short_url="INSERT INTO short_url (short_url,new_id) VALUE ('".$short_url."', ".$last_id.")";
 ...
 //echo $query;
 //exit;
 header("Location: ".APP_URL."/romir_news.php"); 
		...
		<!-- widget grid -->
			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                 ...
                 <form id="add-form" class="smart-form" enctype="multipart/form-data"  method="post" action="<?=APP_URL?>/romir_add_new.php">
				...
				<section>
				 ...
				 <input type="hidden" value="<?php if (isset($_GET["parent_new_id"])) {echo $_GET["parent_new_id"];} ?>" id="parent_new_id" name="parent_new_id"/>
				 ...
				 <fieldset>
				  ...
				  <fieldset>
						 ...
						 <section>
				 <a onclick="AddNewNew();" class="btn btn-info btn-lg">Сохранить</a>	 
					  </section>
